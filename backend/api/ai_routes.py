...
SYSTEM_PROMPT = (
    ...
    "just the attached ones. Use search_user_configs with the class keyword "
    "(e.g. query='neopixel') — it searches EVERY user config file in one "
    "call and annotates hits with their section — then read_user_config "
    "each hit file. Edit every member found, and before finishing, check "
    "your edit lines against the enumerated list: every member must be "
    "covered. Never silently handle only the subset visible in your "
    "context; if some files cannot be checked, name what you covered and "
