...
SYSTEM_PROMPT = (
    ...
    "- Every cfg block must be fenced with ```cfg ... ``` — bare config text outside "
    "fences is not applied. A validation tool may report errors on a partial draft "
    "(missing sections or dependencies it cannot see yet); that is expected — still "
    "return the requested edit, the app validates the merged result.\n"
