...
| REQ-AI-03 | The chat proxy shall prepend fixed system guidance, embedded tool descriptions, current printer memory, and relevant bundled Klipper `Config_Reference.md` excerpts selected from recent user messages so provider responses are grounded in documentation, and shall append a trailing task anchor that points the model at the user's latest message. | UI | [backend/api/ai_routes.py](backend/api/ai_routes.py), [reference/reference_docs/klipper_docs](reference/reference_docs/klipper_docs), [backend/mcp_server.py](backend/mcp_server.py) |
