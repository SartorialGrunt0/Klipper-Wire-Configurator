...
| AI chat API | Provider proxying, docs grounding, embedded MCP tools (native function calling for all providers by default; text ```` ```tool ```` protocol as a per-request escape hatch), stop-event signaling, and tool-mediated config editing behind an approval gate. | [backend/api/ai_routes.py](backend/api/ai_routes.py) |
...
3. Before sending, `ChatDialog.tsx` collects the config files the user explicitly checks in "Include Files" plus user-attached local config files and sends them as `contextFiles` (working state for the backend edit session — nothing is injected into the prompt), then posts through `aiChat` in [frontend/src/services/api.ts](frontend/src/services/api.ts) to `/ai/chat`. Each request carries a `requestId` and a `maxTokens` cap from the chat settings panel. The former handholding injections (regex-targeted section context + file-targeting instructions behind `VITE_KWC_HANDHOLDING`) were deleted in the Phase-5 gate sweep (2026-09): the model discovers content and targets via its tools.
...
6. Tool integration: cloud providers (chatgpt, google, anthropic, github) receive native function-calling tool definitions (`_build_native_tools`) and their responses are parsed from `tool_calls` / `tool_use` blocks (`_extract_native_tool_calls`). Local/OpenAI-compatible providers use native function calling by default as well (native-first flip, 2026-09-17), with the text-based ```` ```tool ```` JSON-block protocol (`_extract_tool_calls`) kept as a fallback parse path and per-request escape hatch (`toolProtocol="text"`). Both paths execute the calls against the embedded MCP server and loop, capped at `MAX_MCP_TOOL_TURNS = 10` (higher for edit sessions); follow-up messages are formatted per provider (`_build_native_tool_followup` / `_build_tool_result_message`). The old no-tool-call doc-search fallback (`_auto_search_context`) was deleted in the Phase-5 gate sweep (2026-09): the loop is intentionally lean-first and the model calls the docs tools itself.
