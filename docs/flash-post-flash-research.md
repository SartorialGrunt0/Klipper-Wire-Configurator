...
4. **Dependency:** flashtool.py needs `pyserial`. **Non-issue in practice** — pyserial is a hard runtime dependency of Klipper itself (`klippy/serialhdl.py` does `import serial`), and KWC's flash feature requires a working Klipper install to build from. The failure only occurs on a host with the Klipper *source* cloned but its Python deps uninstalled, which can't happen in real use. No pre-flight check needed.
