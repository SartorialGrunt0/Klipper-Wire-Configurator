...
export default function SettingsPanel() {
  ...
  const { selectedNodeId, nodes, addSubComponentNode, addFeatureNode, updateNodeData, selectedEdgeId, edges, updateEdgeData } = useGraphStore();
  ...
  const handleParamChange = useCallback(
    (key: string, value: string) => {
      ...
      updateSectionParam(filename, sectionHeader, key, value);
    },
    [configFiles, activeFile, sectionHeader, updateSectionParam],
  );
  ...
  const handleAddParam = useCallback(
    (paramSchema: ParamSchema) => {
      ...
      addParam(filename, sectionHeader, {
    ...
    [configFiles, activeFile, sectionHeader, addParam],
  );
  ...
  const handleRemoveParam = useCallback(
    (key: string) => {
      ...
      removeParam(filename, sectionHeader, key);
    },
    [configFiles, activeFile, sectionHeader, removeParam],
  );
  ...
  const handleAddSubComponent = useCallback((sectionType: string) => {
    ...
    const header = isNamed ? `${sectionType} ${sectionType}_default` : sectionType;

    addSubComponentNode(selectedNodeId, sectionType, label, header);
  ...
  const handleAddFeature = useCallback((sectionType: string) => {
    ...
    addFeatureNode(selectedNodeId, sectionType, label, header);
    const filename = hwData?.configFile || activeFile;
    addSection(filename, {
      section_type: sectionType,
      section_name: isNamed ? `${sectionType}_default` : '',
    ...
    // Keep menu open for multi-select
  }, [selectedNodeId, schemas, nodes, addFeatureNode, addSection, hwData, activeFile]);

  // Toggle primary MCU - only one node can be primary
  const handleTogglePrimary = useCallback(() => {
    if (!selectedNodeId) return;
    const currentIsPrimary = (hwData as Record<string, unknown>)?.isPrimary as boolean;
    if (!currentIsPrimary) {
      nodes.forEach((n) => {
        if (n.type === 'hardware' && n.id !== selectedNodeId) {
          const d = n.data as Record<string, unknown>;
          if (d.isPrimary) updateNodeData(n.id, { isPrimary: false } as Partial<AppNode['data']>);
        }
      });
    }
    updateNodeData(selectedNodeId, { isPrimary: !currentIsPrimary } as Partial<AppNode['data']>);
  }, [selectedNodeId, hwData, nodes, updateNodeData]);
